...
export async function onRequestPost({ request, env }) {
  ...
  if (!token || String(token).trim().length < 8) {
    return Response.json({ ok: false, error: 'Token 格式不正确（长度不足）' }, { status: 400, headers: cors() });
  }

  await env.DB.prepare(
    'INSERT OR REPLACE INTO users (code, pushplus_token) VALUES (?, ?)'
  ).bind(code, String(token).trim()).run();
